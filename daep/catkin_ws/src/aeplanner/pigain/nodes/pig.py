...
class PIGain:
    def __init__(self):
        ...
        self.timer_share = rospy.Timer(rospy.Duration(0.5), self.share_gain_callback)
    ...
    """ Insert nodes from other agents """
    def shared_gain_callback(self, msg):
        if msg.name != self.namespace:
            for node in msg.nodes:
                # Define the position tuple
                pos = (node.position.x, node.position.y, node.position.z)

                # Check if any node already exists at this position
                existing_nodes = list(self.idx.intersection(pos, objects=True))

                # update gain of nodes from other agents
                if existing_nodes:
                    for item in existing_nodes: 
                        # sent in node is lower gain (if higher do nothing)
                        if node.gain < item.object.gain:
                            ...
                            self.idx.insert(self.id, pos, obj=node)
                            self.id += 1  # Increment ID only for new nodes

                # insert new nodes from other agents
                elif not existing_nodes and pos not in self.removed_nodes:  # Only insert if no node exists at this position
                    self.idx.insert(self.id, pos, obj=node)
                    self.id += 1  # Increment ID only for new nodes

    """ Remove nodes cleared by other agents """
    def shared_removed_gain_callback(self, msg):
        if msg.name != self.namespace:
            for node in msg.nodes:
                ...
                if existing_nodes:  # Remove if node exist
                    for item in existing_nodes:
                        self.idx.delete(item.id, (item.object.position.x, item.object.position.y, item.object.position.z))
    ...
    def reevaluate_timer_callback(self, event):
        ...
        for i, item in enumerate(reevaluate_list):
            ...
            if(item.object.gain >= self.node_gain_threshold):
                ...
                self.removed_nodes.remove(pos)
                self.removed_nodes_obj.nodes.remove(item.object)
    ...
    def rviz_callback(self, event):
        ...
        print(self.namespace,len(list(self.idx.intersection(self.bbx, objects=True))))
