...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub, swarm_pos_cmd_pub, rotation_pub, goal_pub;
...
bool receive_rotation_traj_ = false;
double replan_time_;
...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  ...
  receive_rotation_traj_ = false;

}


void rotateDrone(double yaw_end = 0.0) {
  receive_traj_ = false;
  auto t1 = ros::Time::now();
  double tn = 0.0;

  // Get current yaw and normalize both yaws to [-π, π]
  double yaw_start = tf::getYaw(odom.pose.pose.orientation);
  yaw_end = atan2(sin(yaw_end), cos(yaw_end));  // Normalize yaw_end

  double yaw_diff = atan2(sin(yaw_end - yaw_start), cos(yaw_end - yaw_start));

  double yaw_speed = 1; 

  double duration = std::abs(yaw_diff) / yaw_speed;

  // Determine rotation direction (+1 for CCW, -1 for CW)
  double yaw_sign = (yaw_diff >= 0) ? 1.0 : -1.0;
  ...
  while (tn < duration && ros::ok()) {
    ...
    // Compute new yaw based on constant yaw rate
    double yaw = yaw_start + yaw_sign * yaw_speed * tn;
    ...
    cmd.yaw = yaw;
    cmd.yaw_dot = yaw_sign * yaw_speed;
  ...
  if (traj_cmd_.empty() || (pos - traj_cmd_.back()).norm() > 1e-6) {
...
void rotationCallback(const bspline::BsplineConstPtr& msg) {
  if (msg->yaw_pts.empty()) {
    ROS_WARN("Received empty yaw_pts in rotationCallback");
    return;
  }
  double last_yaw = msg->yaw_pts.back(); 
  rotateDrone(last_yaw);
}

void cmdCallback(const ros::TimerEvent& e) {
  // No publishing before receiving traj data
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0) {
    if (receive_rotation_traj_) {
      // Only update yaw, keep position fixed
      pos = traj_[0].evaluateDeBoorT(0.0); // Fixed position
      vel.setZero();
      acc.setZero();
      yaw = traj_[3].evaluateDeBoorT(t_cur)[0];
      yawdot = traj_[4].evaluateDeBoorT(t_cur)[0];
    } else {
      // Follow full trajectory
    ...
    // Current time exceeds range of planned traj
  ...
  double final_yaw = traj_[3].evaluateDeBoorT(traj_duration_)[0];

  double position_error = (pos - final_pos).norm();
  double velocity_norm = vel.norm();
  double yaw_diff = fabs(yaw - final_yaw);  // Absolute difference in yaw

  // Ensure yaw difference is within [-pi, pi] range
  yaw_diff = fmod(yaw_diff + M_PI, 2 * M_PI) - M_PI;

  // if (position_error < 0.1 && fabs(yaw_diff) < 0.1) {  
  //   std_msgs::Bool msg;
  //   msg.data = true;
...
int main(int argc, char** argv) {
  ...
  // manual rotation (no bspline)
  ros::Subscriber rotation_sub = node.subscribe("planning/rotation", 10, rotationCallback);
  rotation_pub = node.advertise<std_msgs::Bool>("/planning/rotation_f",10);
  
  goal_pub = node.advertise<std_msgs::Bool>("/goal_reached",10);
