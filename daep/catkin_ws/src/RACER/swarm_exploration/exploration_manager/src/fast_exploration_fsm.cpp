...
bool waiting_for_goal=true;
bool wait_for_rotation_to_finish_ = true;
bool aeplanner_init = false;
int pos_in_goals;
...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  expl_manager_->ed_->next_poses.clear();
  ...
  // rotation
  rotation_pub_ = nh.advertise<bspline::Bspline>("/planning/rotation", 10);
  rotation_finish_sub_ = nh.subscribe("/planning/rotation_f",1, &FastExplorationFSM::rotationFinishCallback, this);
  ...
  // logging
  pub_start_logging = nh.advertise<std_msgs::Bool>("/start_logging", 10);
  // pub_exploration_complete = nh.advertise<std_msgs::Bool>("/exploration_completed", 10);
  ...
  next_pos_sub_ = nh.subscribe("/goal",1, &FastExplorationFSM::nextPosCallback, this);
  
  // fulhack
  goal_reached_pub_ = nh.advertise<std_msgs::Bool>("/drone/goal_reached", 10);
  switch_to_global_plan_ = nh.advertise<std_msgs::Bool>("/drone/switch_to_global", 10);

  marker_pub = nh.advertise<visualization_msgs::Marker>("/visualization_marker_next_view", 1);

  aeplanner_init_ = nh.subscribe("/aeplanner_init", 1, &FastExplorationFSM::aeplannerInitCallback,this);

  goal_check_timer_ =
      nh.createTimer(ros::Duration(0.2), &FastExplorationFSM::goalCheckCallback, this);

}
void FastExplorationFSM::aeplannerInitCallback(const std_msgs::Bool::ConstPtr& msg)
{
  aeplanner_init = true;
}

// test for following entire branch in RRT
void FastExplorationFSM::goalCheckCallback(const ros::TimerEvent&) {
  if (expl_manager_->ed_->next_poses.empty() || expl_manager_->ed_->next_yaws.empty()) {
    ROS_INFO("No goal poses to process.");
    return;
  }
  
  Vector3d current_pos = fd_->odom_pos_;
  double current_yaw = fd_->odom_yaw_;
  Vector3d target_pos = expl_manager_->ed_->next_poses[pos_in_goals][0];
  double target_yaw = expl_manager_->ed_->next_yaws[pos_in_goals][0];

  // ROS_ERROR_STREAM("Current Position: " << current_pos.transpose()
  //                << " | Current Yaw: " << current_yaw
  //                << " | Target Position: " << target_pos.transpose()
  //                << " | Target Yaw: " << target_yaw
  //                << " | pos_in_goals: " << pos_in_goals);


  double pos_threshold = 1;  // Position threshold for goal reach
  double yaw_threshold = 3;  // Yaw threshold for goal reach

  double yaw_diff = atan2(sin(current_yaw - target_yaw), cos(current_yaw - target_yaw));

  if ((current_pos - target_pos).norm() < pos_threshold && yaw_diff < yaw_threshold) {
    ROS_INFO("Goal reached. Moving to next pose if it exists.");
    ROS_INFO_STREAM("size: "<< expl_manager_->ed_->next_poses.size()<< ", pos: " << pos_in_goals);

    if (expl_manager_->ed_->next_poses.size() - 1 > pos_in_goals) {
      pos_in_goals++;
      expl_manager_->ed_->next_pos = expl_manager_->ed_->next_poses[pos_in_goals][0];
      expl_manager_->ed_->next_yaw = expl_manager_->ed_->next_yaws[pos_in_goals][0];
    } 
  }
  // ROS_ERROR("Next goals:");
  // for (size_t i = 0; i < std::min(expl_manager_->ed_->next_poses.size(), size_t(10)); ++i) {
  //     ROS_ERROR("Goal %ld: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f",
  //              i + 1, expl_manager_->ed_->next_poses[i][0].x(), expl_manager_->ed_->next_poses[i][0].y(), expl_manager_->ed_->next_poses[i][0].z(), expl_manager_->ed_->next_yaws[i][0]);
  // }
  publishNextViewMarker(expl_manager_->ed_->next_pos);

}

void FastExplorationFSM::nextPosCallback(const aeplanner::Goals msg) {
  Eigen::Vector3d goal_position(msg.goals[0].x, msg.goals[0].y, msg.goals[0].z);

  // Check if next_poses is empty or new goal has been sent
  if (expl_manager_->ed_->next_poses.empty() ||
      goal_position != expl_manager_->ed_->next_poses[0][0]) 
  {
      // ROS_WARN("New goal received - new RRT sample");
      // for (size_t i = 0; i < std::min(msg.goals.size(), size_t(10)); ++i) {
      //     ROS_WARN("Goal %ld: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f",
      //              i + 1, msg.goals[i].x, msg.goals[i].y, msg.goals[i].z, msg.goals[i].yaw);
      // }
      pos_in_goals = 0;
      waiting_for_goal = false;

      Vector3d next_pos(msg.goals[0].x, msg.goals[0].y, msg.goals[0].z);
      double next_yaw = msg.goals[0].yaw;

      // set initial value
      expl_manager_->ed_->next_pos = next_pos;
      expl_manager_->ed_->next_yaw = next_yaw;

      // Update next_poses
      expl_manager_->ed_->next_poses.clear();
      for (const auto& goal : msg.goals) {
          expl_manager_->ed_->next_poses.push_back({Vector3d(goal.x, goal.y, goal.z)});
      }

      // Update next_yaws
      expl_manager_->ed_->next_yaws.clear();
      for (const auto& goal : msg.goals) {
          expl_manager_->ed_->next_yaws.push_back({goal.yaw});
      }
...
// debugging
void FastExplorationFSM::publishNextViewMarker(const Eigen::Vector3d& next_pos) {
  ...
  marker.pose.position.x = next_pos.x();
  marker.pose.position.y = next_pos.y();
  marker.pose.position.z = next_pos.z();
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case WAIT_TRIGGER: {
      ...
      // sleep is for issue with aeplanner and sdf map, see aeplanner.cpp
      // manually trigger for automation
  
      while(waiting_for_goal){//} and !aeplanner_init){
        ROS_INFO("waiting for aeplanner and goal");
        ros::spinOnce(); 
        ros::Duration(0.1).sleep();  
      }
      // ROS_INFO("First goal received from planner, starting exploration");
      geometry_msgs::PoseStamped msg;
      triggerCallback(boost::make_shared<geometry_msgs::PoseStamped>(msg));
    ...
    case PLAN_TRAJ: {
      ...
      // ROS_ERROR("Start yaw: %f" ,fd_->start_yaw_(0));

      // Inform traj_server the replanning
      replan_pub_.publish(std_msgs::Empty());
      // std::cout <<"Only rotation needed " << only_rotation_needed() << std::endl;
      // if (only_rotation_needed()){
      //   transitState(MANUAL_ROTATION, "FSM");
      // }
      // else {
      // ROS_ERROR_STREAM("ROTATION: "<<only_rotation_needed());
      int res = callExplorationPlanner();
      // ROS_ERROR_STREAM("Trajectory status: " << planner_manager_->traj_success_);
      if (!planner_manager_->traj_success_){
        // ROS_WARN("Simulating goal reached to sample new tree / goal");
        // std_msgs::Bool msg;
        // msg.data = true;
        ...
        msg.data = true;
        goal_reached_pub_.publish(msg);
        // // switch_to_global_plan_.publish(msg);
      ...
      if (!fd_->go_back_) {
        ...
        if (need_replan) {
          ...
          // if (expl_manager_->updateFrontierStruct(fd_->odom_pos_) != 0) {
            ...
            // std_msgs::Bool msgc;
            // msgc.data = true;
            // pub_exploration_complete.publish(msgc);
          }
        }
      } else {
        // Check if reach goal
    ...
    case MANUAL_ROTATION: {
      callRotationPlanner();
      while (wait_for_rotation_to_finish_){
        ros::spinOnce();  // Process incoming messages
        ros::Duration(0.01).sleep();  // Prevent busy-waiting
      };
      wait_for_rotation_to_finish_ = true;
      // ROS_ERROR("After rot yaw: %f" ,fd_->odom_yaw_);
      transitState(PLAN_TRAJ, "FSM");
...
int FastExplorationFSM::callRotationPlanner() {
  ...
  double yaw_diff = std::fabs(fd_->odom_yaw_ - expl_manager_->ed_->next_yaw);

  if (yaw_diff > M_PI) {  // If yaw difference is greater than pi, adjust to the shortest path
    yaw_diff = 2 * M_PI - yaw_diff;
  }
  // std::cout << "Yaw curr " << fd_->odom_yaw_ << std::endl;
  // std::cout << "Yaw goal " << expl_manager_->ed_->next_yaw << std::endl;
  // std::cout << "Yaw diff " << yaw_diff << std::endl;

  // std::cout << "Pos curr " << fd_->odom_pos_ << std::endl;
  // std::cout << "Pos goal " << expl_manager_->ed_->next_pos << std::endl;

  // Calculate the number of intermediate points based on the yaw difference
  int num_intermediate_points = static_cast<int>(yaw_diff / (M_PI / 8));  // Adjust the step size for resolution
  if (num_intermediate_points < 1) {
    num_intermediate_points = 1;  // Ensure at least one intermediate point
  }

  // std::cout << "Points " << num_intermediate_points << std::endl;

  // Position control points
  std::vector<Eigen::Vector3d> pos_pts;
  pos_pts.push_back(fd_->odom_pos_);  // Start position

  // Yaw control points
  std::vector<double> yaw_pts;
  yaw_pts.push_back(fd_->odom_yaw_);  // Start yaw

  // Generate intermediate positions and yaw points
  for (int i = 1; i <= num_intermediate_points; ++i) {
    double intermediate_yaw = fd_->odom_yaw_ + (expl_manager_->ed_->next_yaw - fd_->odom_yaw_) * (i / static_cast<double>(num_intermediate_points));
    
    Eigen::Vector3d intermediate_pos = fd_->odom_pos_ + (expl_manager_->ed_->next_pos - fd_->odom_pos_) * (i / static_cast<double>(num_intermediate_points));

    pos_pts.push_back(intermediate_pos);
    yaw_pts.push_back(intermediate_yaw);
  }

  pos_pts.push_back(expl_manager_->ed_->next_pos);
  yaw_pts.push_back(expl_manager_->ed_->next_yaw);

  for (const auto& pt : pos_pts) {
    geometry_msgs::Point msg_pt;
    msg_pt.x = pt(0);
    msg_pt.y = pt(1);
    msg_pt.z = pt(2);
    bspline.pos_pts.push_back(msg_pt);
  }

  for (const auto& yaw : yaw_pts) {
    bspline.yaw_pts.push_back(yaw);
  }

  // Define knots for the B-spline (linear B-spline: 2 points per segment)
  Eigen::VectorXd knots(pos_pts.size());
  for (int i = 0; i < pos_pts.size(); ++i) {
    knots(i) = static_cast<double>(i) / (pos_pts.size() - 1);  // Equally spaced knots
  }

  // Add knots to the B-spline
  for (int i = 0; i < knots.size(); ++i) {
    bspline.knots.push_back(knots(i));
  }

  bspline.order = pos_pts.size() - 1;
  bspline.yaw_dt = 1.0;  // Adjust as needed
  
  // planner_manager_->updateTrajInfo();
  bspline.traj_id = info->traj_id_+1;
  info->traj_id_ = info->traj_id_+1;

  fd_->newest_traj_ = bspline;

  rotation_pub_.publish(fd_->newest_traj_);

  
  return 0;
}

int FastExplorationFSM::only_rotation_needed() {
  double threshold = 0.1;  

  Eigen::Vector3d odom_pos = fd_->odom_pos_;
  Eigen::Vector3d next_pos = expl_manager_->ed_->next_pos;

  // std::cout << " rotation check " << odom_pos << std::endl;
  // std::cout << " rotation check " << next_pos << std::endl;

  double distance = (odom_pos - next_pos).norm();
...
void FastExplorationFSM::rotationFinishCallback(const std_msgs::Bool::ConstPtr msg) {
  if (msg->data){
    wait_for_rotation_to_finish_ = false;
  }
}
...
int FastExplorationFSM::callExplorationPlanner() {
  ...
  if (fd_->avoid_collision_ || fd_->go_back_) {  // Only replan trajectory
    ...
    // check if position == goal
    // do not call plan explore motion
    // do roation manually or wait for new goal
...
void FastExplorationFSM::triggerCallback(const geometry_msgs::PoseStampedConstPtr& msg) {
  ...
  // logging
  ROS_WARN_STREAM("Starting logger");
  // LOGGING
  std_msgs::Bool msgl;
  msgl.data = true;
  pub_start_logging.publish(msgl);
