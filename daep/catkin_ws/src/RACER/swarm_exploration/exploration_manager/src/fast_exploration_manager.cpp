...
int FastExplorationManager::planExploreMotion(
  ...
  // } else {
    ...
    // ed_->n_points_.clear();
    // vector<vector<double>> n_yaws = ed_->next_yaws;
    // frontier_finder_->getViewpointsInfo(
    ...
    // std::cout << "n_points: ";
    // for (const auto& point : ed_->n_points_[0]) {
    //     std::cout << "[" << point.x() << ", " << point.y() << ", " << point.z() << "] ";
    // }
    // std::cout << std::endl;
    // std::cout << "refined_points: " << ed_->refined_points_[0] << std::endl;
    // std::cout << "refined_yaws: " << refined_yaws[0] << std::endl;

    // ed_->refined_points_.clear();
    // ed_->refined_views_.clear();
    // vector<vector<double>> n_yaws = ed_->next_yaws;
    // ed_->n_points_ = ed_->next_poses;
    // vector<double> refined_yaws;

    // refineLocalTour(pos, vel, yaw, ed_->n_points_, n_yaws, ed_->refined_points_, refined_yaws);
    // ed_->next_pos = ed_->refined_points_[0];
    // ed_->next_yaw = refined_yaws[0];
  ...
  // }
  ed_->next_pos_ = ed_->next_pos;
  ed_->next_yaw_ = ed_->next_yaw;
  next_pos = ed_->next_pos;
  next_yaw = ed_->next_yaw;

  // std::cout << "Next view: " << next_pos.transpose() << ", " << next_yaw << std::endl;
  // ed_->next_pos_ = next_pos;
  // ed_->next_yaw_ = next_yaw;

  // std::cout << " pos: " << pos << " vel: " << vel << " acc: " 
  // << acc << " yaw: "<< yaw << " next pos: " << next_pos << " next yaw: " << next_yaw << std::endl;
...
int FastExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  double diff0 =  ed_->next_yaw - yaw[0];
  ...
  // ROS_ERROR_STREAM("Distance: " << len);

  // bool optimistic = ed_->plan_num_ < ep_->init_plan_num_;

  // Chain A* searches for all waypoints
  // vector<Vector3d> full_path;
  // Vector3d current_pos = pos;
  // for (size_t i = 0; i < 1; ++i) {
  //   Vector3d next_pos1 = ed_->next_poses[i][0];
  //   double next_yaw = ed_->next_yaws[i][0];

  //   planner_manager_->path_finder_->reset();
  //   if (planner_manager_->path_finder_->search(current_pos, next_pos1, optimistic) != Astar::REACH_END) {
  //     ROS_ERROR("No path to next viewpoint");
  //     return FAIL;
  //   }
    
  //   vector<Vector3d> partial_path = planner_manager_->path_finder_->getPath();
  //   shortenPath(partial_path);
  //   full_path.insert(full_path.end(), partial_path.begin(), partial_path.end());

  //   current_pos = next_pos1;
  // }

  // ed_->path_next_goal_ = full_path;
  // const double radius_far = 7.0;
  // const double radius_close = 1.5;
  // const double len = Astar::pathLength(ed_->path_next_goal_);
  // ROS_ERROR_STREAM("Distance: " << len);
  ...
  // if (true) {
    ...
    // ed_->next_goal_ = ed_->next_poses.back()[0];
    // std::cout << "Close goal." << std::endl;
