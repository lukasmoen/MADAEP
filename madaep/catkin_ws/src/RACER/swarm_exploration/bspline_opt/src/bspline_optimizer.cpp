...
void BsplineOptimizer::optimize() {
  ...
  for (int i = 0; i < point_num_; ++i)
    for (int j = 0; j < dim_; ++j) {
      ...
      if (dim_ != 1) cij = max(min(cij, bmax[j % 3] - 1e-3), bmin[j % 3] + 1e-3);
      q[dim_ * i + j] = cij;
    }
  ...
  if (cost_function_ & MINTIME) {
    // std::cout << "Traj opt iter num: " << iter_num_ << std::endl;
  }
...
void BsplineOptimizer::calcSmoothnessCost(const vector<Eigen::Vector3d>& q, const double& dt,
    double& cost, vector<Eigen::Vector3d>& gradient_q, double& gt) {
  cost = 0.0;
...
void BsplineOptimizer::calcEndCost(const vector<Eigen::Vector3d>& q, const double& dt, double& cost,
  ...
  Eigen::Vector3d zero(0, 0, 0);
  // std::fill(gradient_q.begin(), gradient_q.end(), zero);
  for (int i = q.size() - 3; i < q.size(); ++i) gradient_q[i] = zero;
  gt = 0.0;
...
void BsplineOptimizer::combineCost(
    const std::vector<double>& x, std::vector<double>& grad, double& f_combine) {
  {
    ...
    //   {
    //     for (int j = 0; j < dim_; ++j)
    //       g_q_[order_ + variable_num_ / dim_ + i][j] = control_points_(control_points_.rows() -
    //       order_
    //       + i, j);
    //     for (int j = dim_; j < 3; ++j)
...
double BsplineOptimizer::costFunction(
