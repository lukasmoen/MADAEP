...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  exploration_complete_sub_ = nh.subscribe("/exploration_completed", 1, &FastExplorationFSM::explorationCompletedCallback,this);
...
void FastExplorationFSM::explorationCompletedCallback(const std_msgs::Bool::ConstPtr& msg) 
{
  exploration_complete_ = msg->data;
}
...
void FastExplorationFSM::goalCheckCallback(const ros::TimerEvent&) {
  ...
  if ((current_pos - target_pos).norm() < pos_threshold && yaw_diff < yaw_threshold) {
    ...
    if (expl_manager_->ed_->next_poses.size() - 1 > pos_in_goals) {
      ...
      // Reached final goal: clear remaining goals
      ROS_INFO("Final goal reached. Clearing goal lists.");
      expl_manager_->ed_->next_poses.clear();
...
void FastExplorationFSM::nextPosCallback(const aeplanner::Goals msg) {
     ...
     // goal_position != expl_manager_->ed_->next_poses[0][0]) 
  {
    ...
    prev_goals.goals.clear();
    for (const auto& goal : msg.goals) {
      prev_goals.goals.push_back(goal);
    }
    goal_id = msg.id;
    ...
    case EXEC_TRAJ: {
      ...
      if (!fd_->go_back_) {
        ...
        if (t_cur > fp_->replan_thresh2_ ){//&& expl_manager_->frontier_finder_->isFrontierCovered()) {
          ROS_WARN("Replan: Threshold reached=====================================");
          need_replan = true;
