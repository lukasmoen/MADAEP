...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  ROS_INFO("SDF: Reset map variables");
  // Params of map properties
  double x_size, y_size, z_size;
  // exploration_node_1 is temporary namespace and may cause issues
  nh.param("/exploration_node_1/sdf_map/resolution", mp_->resolution_, -1.0);
  nh.param("/exploration_node_1/sdf_map/map_size_x", x_size, -1.0);
  nh.param("/exploration_node_1/sdf_map/map_size_y", y_size, -1.0);
  nh.param("/exploration_node_1/sdf_map/map_size_z", z_size, -1.0);
  nh.param("/exploration_node_1/sdf_map/obstacles_inflation", mp_->obstacles_inflation_, -1.0);
  nh.param("/exploration_node_1/sdf_map/local_bound_inflate", mp_->local_bound_inflate_, 1.0);
  nh.param("/exploration_node_1/sdf_map/local_map_margin", mp_->local_map_margin_, 1);
  nh.param("/exploration_node_1/sdf_map/ground_height", mp_->ground_height_, 1.0);
  nh.param("/exploration_node_1/sdf_map/default_dist", mp_->default_dist_, 5.0);
  nh.param("/exploration_node_1/sdf_map/optimistic", mp_->optimistic_, true);
  nh.param("/exploration_node_1/sdf_map/signed_dist", mp_->signed_dist_, false);
  nh.param("/exploration_node_1/sdf_map/no_drone_1", mp_->no_drone_1_, false);

  mp_->resolution_ = 0.2;

  ROS_INFO("SDF: Example var resolution: %f",mp_->resolution_);
  ROS_INFO("SDF: Example var map_size_x: %f",x_size);
  ...
  nh.param("/exploration_node_1/sdf_map/p_hit", mp_->p_hit_, 0.70);
  nh.param("/exploration_node_1/sdf_map/p_miss", mp_->p_miss_, 0.35);
  nh.param("/exploration_node_1/sdf_map/p_min", mp_->p_min_, 0.12);
  nh.param("/exploration_node_1/sdf_map/p_max", mp_->p_max_, 0.97);
  nh.param("/exploration_node_1/sdf_map/p_occ", mp_->p_occ_, 0.80);
  nh.param("/exploration_node_1/sdf_map/max_ray_length", mp_->max_ray_length_, -0.1);
  nh.param("/exploration_node_1/sdf_map/virtual_ceil_height", mp_->virtual_ceil_height_, -0.1);
  ...
  ROS_INFO("SDF: Fetching params from ros and setting map specifications");

  // Initialize data buffer of map
  size_t buffer_size = mp_->map_voxel_num_(0) * mp_->map_voxel_num_(1) * mp_->map_voxel_num_(2);
  ROS_INFO("SDF: Buffer size %zu", buffer_size);
  ...
  try {
    ...
    md_->distance_buffer_neg_.resize(buffer_size, mp_->default_dist_);
    md_->distance_buffer_.resize(buffer_size, mp_->default_dist_);
    md_->count_hit_and_miss_.resize(buffer_size, 0);
    md_->count_hit_.resize(buffer_size, 0);
    md_->count_miss_.resize(buffer_size, 0);
    md_->flag_rayend_.resize(buffer_size, -1);
    md_->flag_visited_.resize(buffer_size, -1);
    md_->tmp_buffer1_.resize(buffer_size, 0);
    md_->tmp_buffer2_.resize(buffer_size, 0);
  } catch (const std::bad_alloc& e) {
    ROS_ERROR("Memory allocation failed: %s", e.what());
    ROS_ERROR("Buffer size requested: %zu", buffer_size);
    throw;
  }
  md_->raycast_num_ = 0;
  ...
  ROS_INFO("SDF: setting box boundaries..");
  ...
  ROS_INFO("SDF: setting box boundaries done");
  ...
  ROS_INFO("SDF: setting box vertices");
