...
bool RayCaster::setInput(
    const Eigen::Vector3d& start, const Eigen::Vector3d& end /* , const Eigen::Vector3d& min,
...
bool RayCaster::input(const Eigen::Vector3d& start, const Eigen::Vector3d& end) {
  ...
  // Safe calculation of tDelta values to avoid division by zero
  tDeltaX_ = (dx_ != 0) ? ((double)stepX_) / dx_ : std::numeric_limits<double>::max();
  tDeltaY_ = (dy_ != 0) ? ((double)stepY_) / dy_ : std::numeric_limits<double>::max();
  tDeltaZ_ = (dz_ != 0) ? ((double)stepZ_) / dz_ : std::numeric_limits<double>::max();
  
  // Calculate tMax values (with safe checks)
  tMaxX_ = (dx_ != 0) ? intbound(start_.x(), dx_) : std::numeric_limits<double>::max();
  tMaxY_ = (dy_ != 0) ? intbound(start_.y(), dy_) : std::numeric_limits<double>::max();
  tMaxZ_ = (dz_ != 0) ? intbound(start_.z(), dz_) : std::numeric_limits<double>::max();
