...
void LocalExploreFSM::init(ros::NodeHandle& nh) {
  ...
  safety_timer_ =
      nh.createTimer(ros::Duration(0.05), &LocalExploreFSM::checkCollisionCallback, this);
...
bool LocalExploreFSM::callKinodynamicReplan() {
  ...
  if (plan_success) {
    ...
    visualization_->drawBspline(info->position_traj_, 0.1, Eigen::Vector4d(1.0, 1.0, 0.0, 1), true,
