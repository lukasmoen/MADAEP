...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d& start_pt,
  ...
  // ROS_INFO("Kino t: %lf, opt: %lf", t_search, t_opt);

  updateTrajInfo();
  return true;
}

void FastPlannerManager::planExploreTraj(vector<Eigen::Vector3d>& tour,
  ...
  // if (tour.size() == 1) {
  //   ROS_WARN("Only one waypoint provided. Generating an additional midpoint.");
    
  //   // Create a point slightly in front of the only existing waypoint
  //   Eigen::Vector3d extra_point = tour[0] + Eigen::Vector3d(0.1, 0.1, 0.0); 
  //   tour.push_back(extra_point);
  // }
  ...
  if (pos.rows() == 0 || pos.cols() == 0) {
    ROS_ERROR("Trajectory position matrix is empty!");
  }
  if (times.size() != pt_num - 1) {
    ROS_ERROR("Time vector has incorrect size: %ld, expected: %d", times.size(), pt_num - 1);
  }
  if (pt_num < 2) {
    ROS_ERROR("Not enough waypoints to generate a trajectory.");
    // resample rrt
    traj_success_ = false;
    return;
  }
