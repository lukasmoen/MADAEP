...
#include "rpl_exploration/Goals.h"
...
void switchToGlobalCallback(const std_msgs::Bool::ConstPtr& msg)
{
...
{
    // std::cout << "msg data " << static_cast<int>(msg->data) << std::endl;
    ...
    else
    {
      if (msg->data != prev_goal_reached)
      {
        ...
        // double check, first done in traj_server (motion planner)
        // double distance = 
        // sqrt(pow(current_pose_stamped.pose.position.x - goals_.goals[0].x, 2) + 
        // pow(current_pose_stamped.pose.position.y - goals_.goals[0].y, 2) + 
        // pow(current_pose_stamped.pose.position.z - goals_.goals[0].z, 2));
...
*/
void publishGoalAndWait(ros::Publisher goal_pub, int const& freq, rpl_exploration::Goals& goals){
  ...
  if (goals.goals.size() == 1) {
    std::cout << "Goal only yaw" << std::endl;
    // Generate a random shift within a 10 cm radius
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> angle_dist(-1, 1);
    std::uniform_real_distribution<double> radius_dist(0.5, 0.5); // Max shift of 10 cm
    ...
    rpl_exploration::Goals goals_modified = goals;

    goals_modified.goals[0].x += dx;
    goals_modified.goals[0].y += dy;

    goals = goals_modified;

  }

  // if (goals.goals != goals_.goals) {
  //   ROS_ERROR("NEW GOAL");
  // }
  ...
  //   rpl_exploration::Goals single_goal;
  //   single_goal.goals.push_back(goals.goals[0]);  // Keep only the first goal
  //   goals = single_goal;
  // } else {
  //     ROS_WARN("Goals list is empty, cannot assign first goal.");
  // }

  // If there are multiple goals, add intermediate goals
  // if (goals.goals.size() > 1) {
  //   rpl_exploration::Goals enhanced_goals;
    
  //   // Always add an intermediate goal in the middle of current existing goals
  //   for (size_t i = 0; i < goals.goals.size() - 1; i++) {
  //     // Add the current goal
  //     enhanced_goals.goals.push_back(goals.goals[i]);
      
  //     // Calculate vector from current to next goal
  //     double dx = goals.goals[i+1].x - goals.goals[i].x;
  //     double dy = goals.goals[i+1].y - goals.goals[i].y;
  //     double dz = goals.goals[i+1].z - goals.goals[i].z;
      
  //     // Calculate yaw (direction to the next goal)
  //     double yaw = atan2(dy, dx);
      
  //     // Create intermediate goal exactly in the middle (ratio = 0.5)
  //     rpl_exploration::Goal intermediate;
  //     intermediate.x = goals.goals[i].x + dx * 0.5;
  //     intermediate.y = goals.goals[i].y + dy * 0.5;
  //     intermediate.z = goals.goals[i].z + dz * 0.5;
      
  //     // Yaw points to the next goal
  //     intermediate.yaw = yaw;
      
  //     // Add the intermediate goal
  //     enhanced_goals.goals.push_back(intermediate);
  //   }
  //   // Add the final goal
  //   enhanced_goals.goals.push_back(goals.goals.back());
    
  //   // Replace original goals with enhanced goals
  ...
  while (ros::ok())
    {
      ...
      // should prob be goal_reached not prev_goal...

      if (goal_reached)
      {
            // ROS_ERROR_STREAM("Goal " << goal_reached);

            geometry_msgs::Pose final_pose;
            final_pose.position.x = goals.goals.back().x;
            final_pose.position.y = goals.goals.back().y;
            final_pose.position.z = goals.goals.back().z;
...
int main(int argc, char** argv)
{
  ...
  //     ROS_INFO("Model name retrieved globally: %s", model_name.c_str());
  // } else {
  //     ROS_WARN("Failed to get global param '/model_name'. Using default.");
  //     model_name = "none";
  // }
  ...
  rpl_exploration::Goals goals;
  // ros::Publisher goal_pub = nh.advertise<rpl_exploration::Goal>("goal", 0);
  ros::Publisher goals_pub = nh.advertise<rpl_exploration::Goals>("goal", 0);
  ...
  // motion planner communication
  ros::Subscriber switch_to_global_plan = nh.subscribe("/switch_to_global",10,switchToGlobalCallback);

  ros::Subscriber aeplanner_init_ = nh.subscribe("/aeplanner_init", 1, aeplannerInitCallback);
  ...
  exploration_completion_pub = nh.advertise<std_msgs::Bool>(ns+"/exploration_completed", 1);
  ...
  //   else
  //   {
  //     ROS_ERROR("7Failed to call service /get_coverage");
  //   }
  // logfile << 0 << ", " 
  //         << 0 << ", "
  //         << 0 << ", "
  ...
  publishGoalAndWait(goals_pub, GOAL_PUB_FREQ, goals);
  ROS_WARN("first rotation");
  goal.yaw = 4;
  goals.goals.clear();
  goals.goals.push_back(goal);
  publishGoalAndWait(goals_pub, GOAL_PUB_FREQ, goals);
  ROS_WARN("second rotation");
  goal.yaw = 0;//tf2::getYaw(init_pose->pose.orientation);
  goals.goals.clear();
  goals.goals.push_back(goal);
  publishGoalAndWait(goals_pub, GOAL_PUB_FREQ, goals);
  ROS_WARN("third rotation");
  ...
  while (ros::ok())
  {
    ...
    {    
      ...
      goals.goals.clear();
      geometry_msgs::PoseArray goals_ = aep_ac.getResult()->poses;
      for (const auto& goal_pose : goals_.poses) {
    ...
    // else 
    // {
    //   ROS_ERROR("8Failed to call service /get_coverage");
    // }
  }
  ...
  //   else 
  //   {
  //     ROS_ERROR("9Failed to call service /get_coverage");
  // }
  // Completion
  collision_pub.publish(msg);
