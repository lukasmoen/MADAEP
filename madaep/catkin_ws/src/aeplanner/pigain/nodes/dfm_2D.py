...
class DynamicFrequencyMap:
    def __init__(self):
        ...
        if self.visualizeDFM:
            ...
            self.im = self.ax.imshow(self.grid, cmap='RdYlBu', vmin=-1, vmax=1, origin='upper', extent=[0, self.y_size, 0, self.x_size])
    ...
    def normalize_grid(self):
        max_abs_val = max(abs(np.max(self.grid)), abs(np.min(self.grid)))
        if max_abs_val != 0:
            normalized_grid = self.grid / max_abs_val
        else:
    ...
    def update_timer_callback(self, timer):
        ...
        for agent in self.agents.values():
            observation = (agent.position.x, agent.position.y)
            x = int((observation[0] - self.round_min_x) / self.square_size)
            y = int((observation[1] - self.round_min_y) / self.square_size)
            self.grid[x, y] -= 1
